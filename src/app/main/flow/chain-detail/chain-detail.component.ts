...
export class ChainDetailComponent implements OnDestroy, OnInit {
  ...
  private currentBiz = '';
  private removedChainActResultIds: string[] = [];
  ...
  async onSaveChainAct() {
              ...
              nextActions: result?.nextActions?.map((nextAction) => {
                const modify = {
                  ...
                  delayValue: nextAction.delayValue
                    ...
                    : null,
                  addNewChain: {
                    chainId: nextAction.addNewChain?.chainId,
    ...
    const bodyDelay = {
      ...
      subActionIds: this.detailChain?.actionResults?.map((actResult) => {
        return actResult.subActions?.map((subAction) => subAction.id) || [];
      }) || [],
  ...
  onRemoveAction() {
    ...
    return new Promise((resolve, reject) => {
      this.removedChainActResultIds.forEach((id) => {
        this.autoTaskService.chainAction
          .deleteChainAct(this.detailChain!.id, id!)
          .subscribe({
            next: (res) => {
              if (res.status === 200) {
                this.removedChainActResultIds =
                  this.removedChainActResultIds.filter(
                    (id) => id !== res.data.id,
                  );
              } else {
                this.commonService.handleResErr(res);
