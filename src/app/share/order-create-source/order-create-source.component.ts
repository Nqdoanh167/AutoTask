...
import { EntityPagination, OrderPlatformSource, Source } from 'src/app/types/viewmodels';

interface TypeSource {
  id: string, name: string, platformId: string;
  link?: string;
  platform: string; picture: string, count: number, isHover: boolean, rows: Source[]
}
...
export class OrderCreateSourceComponent implements OnInit, OnDestroy {
  ...
  onSelect(typeSource: TypeSource, source?: Source) {
    ...
    this.values = [...this.listTypeSource, ...this.source.rows].filter(v => this.valueIds.includes(v.id)).map(v => ({
      id: v.id,
      name: v.name,
      link: v.link,
      picture: v.picture,
