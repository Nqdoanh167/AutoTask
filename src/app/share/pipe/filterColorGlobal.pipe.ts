...
export class FilterColorGlobalPipe implements PipeTransform {
  statuses= [
    ...
    { label: 'Waiting', value: 'waiting', bgColor: '#fd35cf', txtColor: '#17234e' },
    { label: 'Doing', value: 'doing', bgColor: '#ff6700', txtColor: '#17234e' },
    { label: 'Completed', value: 'completed', bgColor: '#00a200', txtColor: '#17234e' },
    { label: 'Trash', value: 'trash', bgColor: '#252a31', txtColor: '#17234e' },
  ];
  constructor() {}
  transform(code: string, type: string = 'bgColor', defaultColor?: string): any {
    ...
    if (item) {
      if (type === 'style') {
        return {
          backgroundColor: `rgb(${this.hex2rgb(item.bgColor || '').join(',')},0.2)`,
          color: item.bgColor
        };
      }
      if (type === 'styleCellCalendar') {
